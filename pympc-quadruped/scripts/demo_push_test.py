...
运行这个脚本可以在 MuJoCo viewer 中看到：
1. 机器人正常行走
2. 在 5 秒时施加侧向推力（会看到机器人被推）
3. 机器人尝试恢复平衡
"""
...
def run_push_test_demo():
    ...
    duration = 15.0  # 延长到 15 秒，更好观察
    num_steps = int(duration / dt)

    # 推力参数
    push_time = 5.0  # 5 秒时施加推力
    push_duration = 0.1  # 持续 0.1 秒
    push_force = 40.0  # 40 N
    ...
    print("  🟠 橙色箭头: 推力方向")
    print("\n观察要点:")
    print("  1. 前 5 秒: 机器人正常行走")
    print("  2. 5.0-5.1 秒: 施加侧向推力（机器人会被推向一侧）")
    print("  3. 5.1 秒后: 观察机器人如何恢复平衡")
    print("="*80 + "\n")
    ...
    if len(body_trajectory) > 1:
        total_distance = np.linalg.norm(body_trajectory[-1][:2] - body_trajectory[0][:2])
        print(f"总行走距离: {total_distance:.2f} m")
    print("="*80)
...
if __name__ == "__main__":
    print("\n启动推力测试演示...")
    print("请观察 MuJoCo viewer 中的机器人行为\n")
