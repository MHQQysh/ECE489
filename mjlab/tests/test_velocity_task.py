...
from mjlab.asset_zoo.robots import G1_ACTION_SCALE, GO1_ACTION_SCALE, GO2_ACTION_SCALE
...
def test_go2_terrain_variants_are_registered(
  go2_velocity_task_ids: list[str],
) -> None:
  """Go2 should expose selectable flat, rough, and focused terrain variants."""
  assert set(go2_velocity_task_ids) >= {
    "Mjlab-Velocity-Flat-Unitree-Go2",
    "Mjlab-Velocity-Rough-Unitree-Go2",
    "Mjlab-Velocity-Slope-Unitree-Go2",
    "Mjlab-Velocity-Stairs-Unitree-Go2",
    "Mjlab-Velocity-Bumps-Unitree-Go2",
    "Mjlab-Velocity-Random-Unitree-Go2",
  }


def test_go2_focused_terrain_variants_have_generators() -> None:
  """Focused Go2 terrain variants should use the expected sub-terrain set."""
  expected_sub_terrains = {
    "Mjlab-Velocity-Slope-Unitree-Go2": {"flat", "slope_up", "slope_down"},
    "Mjlab-Velocity-Stairs-Unitree-Go2": {
      "flat",
      "easy_stairs",
      "stairs_up",
    ...
    "Mjlab-Velocity-Bumps-Unitree-Go2": {"flat", "low_bumps", "tall_bumps"},
    "Mjlab-Velocity-Random-Unitree-Go2": {"flat", "random_hfield"},
  }

  for task_id, sub_terrains in expected_sub_terrains.items():
    cfg = load_env_cfg(task_id)

    assert cfg.scene.terrain is not None, f"Task {task_id} has no terrain config"
    assert cfg.scene.terrain.terrain_type == "generator"
    assert cfg.scene.terrain.terrain_generator is not None
    assert set(cfg.scene.terrain.terrain_generator.sub_terrains) == sub_terrains
